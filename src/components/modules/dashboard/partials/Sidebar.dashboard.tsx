...
import {
  DASHBOARD_NAV_ITEMS,
  type NavItemType,
} from "../../../../routes/dashboard/Dashboard.nav-items.tsx";
...
export const SidebarDashboard = () => {
  const navGroup = useMemo(
    () =>
      DASHBOARD_NAV_ITEMS.map((navGroupItems, key) => (
        <div key={key}>
          <Text c="dimmed">{navGroupItems.group}</Text>
          {navGroupItems.items.map((navItem) => (
            <NavItem
              icon={navItem.icon}
              label={navItem.label}
              path={navItem.path}
  ...
  return (
    <>
      <Stack className="mt-2">{navGroup}</Stack>
    </>
  );
};
